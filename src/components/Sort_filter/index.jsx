import { Box, Typography, Modal, Button, Autocomplete, TextField} from "@mui/material";
...
function SortFilter(props) {
    ...
    const continentOption = Array.from(new Set(data.map((venue) => venue.location.continent))).filter((value) => value !== "");

    useEffect(() => {
        if(continent){
            // eslint-disable-next-line
            setNewData(data.filter((venue) => venue.location.continent === continent));
        }else{
            setNewData(data);
        }
        // eslint-disable-next-line
    ...
    const saveFilter = () => {
        ...
        if(continentValue || country || sortValue){
            if(continentValue && country && !sortValue){
                ...
                newUrl(`?&filter=true&continent=${continentValue}&price=${sortValue.value}`);
            }else{
                newUrl(`?&filter=true&continent=${continentValue}&country=${country}&price=${sortValue.value}`);
            }
        }else{
            newUrl("");
        }
    ...
    const countryOption = Array.from(new Set(newData.map((country) => country.location.country))).filter((value) => value !== "");
    ...
    return (
        <div>
            <Button variant="outlined" endIcon={<TuneOutlinedIcon />} sx={{
                mb: 1,
                background: theme.palette.light.main,
                borderColor: theme.palette.grey.main,
                fontWeight: "bold",
            }} onClick={handleOpen}>Sort & Filter</Button>
      <Modal
        open={open}
        onClose={handleClose}
      ...
      >
        <Box sx={style}>
            <Box>
                ...
                sx={{ width: 300, border: "1px solid", borderColor: theme.palette.primary.main, borderRadius: "10px"}}
            ...
            <Box sx={{textAlign: "center", mt:3,}}>
                <Button variant="contained" onClick={saveFilter} sx={{width: 100, borderRadius: "20px", fontWeight: "bold"}}>Save</Button>
            </Box>
        </Box>
      </Modal>
        </div>
